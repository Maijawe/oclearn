...
app.get("/api/startgame",authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`levelWords: ${levelWords.length}, streak: ${streak}, stars: ${stars}, cipherKeys: ${cipherKeys}, level: ${level},currentDailyStars: ${currentDailyStars} , highestStars: ${highestStars}`);

    res.json({ levelWords, streak, cipherStreak, stars, cipherKeys, level ,highestStars });
  } catch (error) {
